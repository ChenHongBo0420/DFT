...
def main():
    ...
    if args.mode == "train":
        ...
        # ─── 计算 padding_size: max_atom_count * padding_multiplier ───
        max_train = get_max_atom_count(train_folders)
        max_val = get_max_atom_count(val_folders)
        padding_size = int(max(max_train, max_val) * args.padding_multiplier)
        ...
        if args.predict_chg:
            chg_ckpt = "best_chg.pth"
            chg_model = load_pretrained_chg_model(
                chg_ckpt,
        ...
        # ─── 2) 如果要预测能量 ───
        if args.predict_energy:
            # **注意：模型文件实际上叫 “newEmodel.pth”，不是 best_energy.pth**
            energy_ckpt = "newEmodel.pth"
            # 先计算 dim_C...：和训练时一致
            # 这里我们假设 “fingerprint_dim = 360, basis_dim = 9” （和 train_energy_model 里同样逻辑）
            fingerprint_dim = 360
            basis_dim       = 9
